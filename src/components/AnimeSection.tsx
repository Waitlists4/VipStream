...
import { Search, TrendingUp, Film, Star, ArrowLeft, ArrowRight, ChevronsLeft, ChevronsRight, Tv } from 'lucide-react';
...
  
  // Use separate states for content and search loading
  const [contentLoading, setContentLoading] = useState(true);
  const [searchLoading, setSearchLoading] = useState(false);

  // Separate states for TV shows and movies
...
  // Load initial data
  useEffect(() => {
    // Only fetch content if there's no active search
    if (!query) {
...
                        <Tv className={`inline-block mr-2 text-purple-500 ${isMobile ? 'w-5 h-5' : 'w-6 h-6'}`} />
                        TV Shows & Series
                      </h3>
                      <div className={`grid gap-6 ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6'}`}>
...
                    <div>
                      <h3 className={`font-semibold mb-4 text-gray-800 dark:text-gray-200 ${isMobile ? 'text-base' : 'text-xl'}`}>
                        <Film className={`inline-block mr-2 text-pink-500 ${isMobile ? 'w-5 h-5' : 'w-6 h-6'}`} />
                        Movies
                      </h3>
                      <div className={`grid gap-6 ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6'}`}>
                        {popularMovies.map(anime => <AnimeCard key={anime.id} anime={anime} />)}
                      </div>
                    </div>
                  )}
                </div>
...
                      </h3>
                      <div className={`grid gap-6 ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6'}`}>
                        {trendingMovies.map(anime => <AnimeCard key={anime.id} anime={anime} />)}
                      </div>
                    </div>
                  )}
                </div>
