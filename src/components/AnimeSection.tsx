...
const AnimeSection: React.FC = () => {
  ...
  // Fetch initial data (popular/trending) on component mount or when search is cleared
  useEffect(() => {
  ...
  // Perform search when `initialQuery` (from URL) or `currentPage` changes
  useEffect(() => {
  ...
  // Separate effect for fetching suggestions
  ...
  const handleSuggestionClick = (anime: Anime) => {
    clearSuggestions();
    const path = `/anime/tv/${anime.id}`;
    navigate(path);
  };

  // Memoized search results and pagination data
  const searchTvShows = useMemo(() => searchResults.filter(anime => !anilist.isMovie(anime)), [searchResults]);

  const totalPages = Math.ceil(searchTvShows.length / ITEMS_PER_PAGE);
      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${isMobile ? 'pt-8 pb-8' : 'pt-16 pb-12'}`}>
        ...
        <div className="text-center mb-12">
            ...
            <p className="text-lg sm:text-xl text-gray-600 dark:text-gray-300 mb-6 sm:mb-8 max-w-3xl mx-auto transition-colors duration-300 px-4">
              LunaStream is your home for all the best anime series past and present.
            </p>
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className={`font-bold text-gray-900 dark:text-white ${isMobile ? 'text-xl' : 'text-2xl'}`}>
                Search Results for "<span className="text-pink-600 dark:text-pink-400">{initialQuery}</span>"
              </h2>
              <p className="text-gray-600 dark:text-gray-400">
                {searchTvShows.length} {searchTvShows.length === 1 ? 'result' : 'results'}
              </p>
            </div>
            ...
            {searchLoading ? (
              ...
              <>
                ...
                <div className="mb-8">
                  <div className={`grid gap-6 ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6'}`}>
                    {searchTvShows.map(anime => <AnimeCard key={anime.id} anime={anime} />)}
                  </div>
                </div>
          ...
          <>
            {contentLoading ? (
              <div className="text-center text-gray-500 dark:text-gray-400 py-12">
                <Loading message="Loading popular and trending anime..." />
              </div>
            ) : (
              <>
                {/* Popular Section */}
                {popularTvShows.length > 0 && (
                  <div className="mb-12">
                    <h2 className={`flex items-center mb-8 font-bold text-gray-900 dark:text-white ${isMobile ? 'text-xl' : 'text-3xl'}`}>
                      <Star className={`mr-3 text-purple-500 ${isMobile ? 'w-6 h-6' : 'w-8 h-8'}`} />
                      Popular Anime
                    </h2>
                    <div className={`grid gap-6 ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6'}`}>
                      {popularTvShows.map(anime => <AnimeCard key={anime.id} anime={anime} />)}
                    </div>
                  </div>
                )}
                ...
                {trendingTvShows.length > 0 && (
                  <div className="mb-12">
                    <h2 className={`flex items-center mb-8 font-bold text-gray-900 dark:text-white ${isMobile ? 'text-xl' : 'text-3xl'}`}>
                      <TrendingUp className={`mr-3 text-pink-500 ${isMobile ? 'w-6 h-6' : 'w-8 h-8'}`} />
                      Trending Anime
                    </h2>
                    <div className={`grid gap-6 ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6'}`}>
                      {trendingTvShows.map(anime => <AnimeCard key={anime.id} anime={anime} />)}
                    </div>
                  </div>
                )}
              </>
